...
mod mint {
	...
	fn test_transaction_is_ok() {
		...
		let level: String = Default::default();
		let short_description: String = Default::default();
		let expiry: u64 = Default::default();
		...
		let (submem_pubkey, _) = Pubkey::find_program_address(
			&[b"submem", mint_info.pubkey().as_ref()],
			&program_id,
		);
		let accounts = vec![
			AccountMeta::new(fee_payer_info.pubkey(), true),
			AccountMeta::new(mint_info.pubkey(), true),
			AccountMeta::new(submem_pubkey, false),
