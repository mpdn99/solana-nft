...
/// 2. `[writable]` submem: [SubmemMetadata] 
...
/// - expiry: [u64] 
	pub fn process_mint(
		...
		let submem_info = next_account_info(account_info_iter)?;
		...
		// Derive PDAs
		let (submem_pubkey, submem_bump) = Pubkey::find_program_address(
			&[b"submem", mint_info.key.as_ref()],
			program_id,
		);
		...
		if *submem_info.key != submem_pubkey {
		...
		invoke_signed(
			...
			&[fee_payer_info.clone(), submem_info.clone()],
			&[&[b"submem", mint_info.key.as_ref(), &[submem_bump]]],
		)?;
		...
		if submem_info.data_len() != SubmemMetadata::LEN {
			return Err(NftError::InvalidAccountLen.into());
		}
		...
		let submem = &mut AccountPDA::new(
			&submem_info,
			try_from_slice_unchecked::<SubmemMetadata>(&submem_info.data.borrow()).unwrap(),
			submem_bump,
		);
		...
		submem.data.serialize(&mut &mut submem_info.data.borrow_mut()[..])?;
